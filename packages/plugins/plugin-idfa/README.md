...
`Plugin` which retrieves IDFA (Identifier for Advertisers) data on iOS devices. IDFA data will be included in `event` payloads under `event.context.device`.
...
### Customizing IDFA Plugin Initialization

To delay the `IDFA Plugin` initialization (i.e., to avoid race condition with push notification prompt) implement the following:
