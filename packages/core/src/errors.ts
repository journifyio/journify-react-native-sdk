 ...
 */
export class JournifyError extends Error {
  type: ErrorType;
  message: string;
  innerError?: unknown;
  ...
  constructor(type: ErrorType, message: string, innerError?: unknown) {
    super(message);
    Object.setPrototypeOf(this, JournifyError.prototype);
    this.type = type;
...
/**
 * Custom Error type for Journifyio HTTP Error responses
 */
export class NetworkError extends JournifyError {
  statusCode: number;
  type:
 ...
 */
export class JSONError extends JournifyError {
  constructor(
 ...
 */
export const translateHTTPError = (error: unknown): JournifyError => {
  // SegmentError already
  if (error instanceof JournifyError) {
    return error;
